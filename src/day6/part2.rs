...
mod test {
    ...
    #[test]
    fn example() {
        let blocks = vec![2, 4, 1, 2];
        let mem_bank = MemBank::new(blocks);
        assert_eq!(mem_bank.iter().count() - 1, 4);
    }
}
...
fn main() {
    let blocks: Vec<u32> = include_str!("question").split_whitespace()
                                                   ...
                                                   .collect();
    let mem_bank = MemBank::new(blocks);
    let cycle_blocks = mem_bank.iter().last().unwrap();
    let loop_mem_bank = MemBank::new(cycle_blocks);
    println!("{}", loop_mem_bank.iter().count() - 1);
}
